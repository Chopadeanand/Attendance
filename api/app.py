...
# Store processed data in memory (for Vercel compatibility)
processed_data = {
    'xlsx_buffer': None,
    'docx_buffer': None,
    'pdf_buffers': [],
...
def download_xlsx():
    ...
    xlsx_buffer = processed_data['xlsx_buffer']
    xlsx_buffer.seek(0)
...
def download_docx():
    ...
    docx_buffer = processed_data['docx_buffer']
    docx_buffer.seek(0)
...
def download_pdfs_zip():
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
        for pdf_info in processed_data['pdf_buffers']:
            zf.writestr(pdf_info['filename'], pdf_info['data'])
    zip_buffer.seek(0)
