"""
Serverless-compatible employee data processor.
Processes Excel file content from memory and returns employees data + XLSX buffer.
"""
import io
import pandas as pd
import openpyxl
...
# Aliases for project codes
PROJECT_ALIASES = {
    "JMTPL":  "JMTL", "KETPL":  "KETL", "KMTPL":  "KMTL",
    "NKTPL":  "NKTL", "SMTPL":  "SMTL", "GAEPL":  "GAPEL",
    "MSHP":   "MSHPL", "NAM":    "NAMEL", "SIPL":   "SIPL",
    "SPPL":   "SPPL", "WVEL":   "WVEPL",
}

# Combined WBS column order
WBS_COLUMN_ORDER = [
    "APEL-SSA-TAR-ATA-HIRATE-MC-199", "BFHL-SSA-TAR-ATA-HIRATE-MC-210",
    "BWHPL-SSA-TAR-ATA-HIRATE-MC-340", "DATL-SSA-TAR-ATA-HIRATE-MC-198",
    "FRHL-SSA-TAR-ATA-HIRATE-MC-197", "GAPEL-SSA-TAR-ATA-HIRATE-MC-200",
    "JMTL-SSA-TAR-ATA-HIRATE-MC-194", "KETL-SSA-TAR-ATA-HIRATE-MC-196",
    "KMTL-SSA-TAR-ATA-HIRATE-MC-195", "KTIPL-SSA-TAR-ATA-HIRATE-MC-348",
    "MBEL-SSA-TAR-ATA-HIRATE-MC-192", "MHPL-SSA-TAR-ATA-HIRATE-MC-346",
    "MKTL-SSA-TAR-ATA-HIRATE-MC-191", "MSHPL-SSA-TAR-ATA-HIRATE-MC-343",
    "NAMEL-SSA-TAR-ATA-HIRATE-MC-400", "NDEPL-SSA-TAR-ATA-HIRATE-MC-190",
    "NKTL-SSA-TAR-ATA-HIRATE-MC-189", "SIPL-SSA-TAR-ATA-HIRATE-MC-350",
    "SMTL-SSA-TAR-ATA-HIRATE-MC-188", "SPPL-SSA-TAR-ATA-HIRATE-MC-352",
    "WVEPL-SSA-TAR-ATA-HIRATE-MC-193", "WUPTL-SSA-TAR-ATA-HIRATE-MC-187",
    "APEL-SSA-TAR-ASA-E. NSV Analysis-MC-139", "BFHL-SSA-TAR-ASA-E. NSV Analysis-MC-385",
    "BWHPL-SSA-TAR-ASA-E. NSV Analysis-MC-341", "DATL-SSA-TAR-ASA-E. NSV Analysis-MC-148",
    "FRHL-SSA-TAR-ASA-E. NSV Analysis-MC-147", "GAEPL-SSA-TAR-ASA-E. NSV Analysis_MC_232",
    "JMTL-SSA-TAR-ASA-E. NSV Analysis-MC-146", "KETL-SSA-TAR-ASA-E. NSV Analysis-MC-141",
    "KMTL-SSA-TAR-ASA-E. NSV Analysis-MC-149", "KTIPL-SSA-TAR-ASA-E. NSV Analysis-MC-347",
    "MBEL-SSA-TAR-ASA-E. NSV Analysis-MC-145", "MHPL-SSA-TAR-ASA-E. NSV Analysis-MC-345",
    "MKTL-SSA-TAR-ASA-E. NSV Analysis-MC-140", "MSHPL-SSA-TAR-ASA-E. NSV Analysis-MC-344",
    "NAMEL-SSA-TAR-ASA-E. NSV Analysis-MC-152", "NDEPL-SSA-TAR-ASA-E. NSV Analysis-MC-138",
    "NKTL-SSA-TAR-ASA-E. NSV Analysis-MC-143", "SIPL-SSA-TAR-ASA-E. NSV Analysis-MC-349",
    "SMTL-SSA-TAR-ASA-E. NSV Analysis-MC-142", "SPPL-SSA-TAR-ASA-E. NSV Analysis-MC-351",
    "WVEL-SSA-TAR-ASA-E. NSV Analysis-MC-150", "WUPTL-SSA-TAR-ASA-E. NSV Analysis-MC-144",
...
def parse_input(excel_content):
    """Parse employee attendance data from Excel content."""
    ...
    sheet1_name = resolve_sheet(all_sheets,
        ['Sheet1', 'sheet1', 'Attendance', 'Input', 'Data', 'EmpData'], 0)
    
    raw = pd.read_excel(io.BytesIO(excel_content), sheet_name=sheet1_name, header=None)
...
def parse_sheet2(excel_content):
    """Parse employee details/bank info from Excel content."""
    wb = openpyxl.load_workbook(io.BytesIO(excel_content), read_only=True, data_only=True)
    all_sheets = wb.sheetnames
    wb.close()
    ...
    sheet2_name = resolve_sheet(all_sheets,
        ['Sheet2', 'sheet2', 'EmployeeDetails', 'Employee Details', 'BankDetails',
         'Bank Details', 'Details', 'Providers', 'ServiceProviders'], 1)
    
    df = pd.read_excel(io.BytesIO(excel_content), sheet_name=sheet2_name, dtype=str)
    records = {}
...
def process_employee_data(excel_content):
    """
    Main function that processes Excel content in memory.
    Returns: (employees_list, xlsx_buffer)
    """
    employees = parse_input(excel_content)
    sheet2 = parse_sheet2(excel_content)
    ...
    # Create employee list with all details for invoices
    MONTHLY, MONTH_DAYS = 16500, 28
    rate = MONTHLY / MONTH_DAYS
    emp_list = []
